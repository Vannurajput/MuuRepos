...
import { MoTextProps, FileTab, EditorState, ContextMenuState, UISettings, Project, FindState, WhitespaceVisibility, Version, EditorPane, FileContextMenuState, ProjectHistoryEvent, ProjectSnapshot, ProjectAnalysisReport, DeepAnalysisReport, ProjectSearchState, SearchResult, GitState, TestResult } from './types';
...
import { getLanguageSupport, getCommonLanguages } from './languages';
...
const MoText: React.FC<MoTextProps> = ({ initialData, onSave }) => {
  ...
  const [gitState] = useState<GitState>({ status: [], processingAction: 'none', processingMessage: null });
  ...
  const handleContentChange = useCallback((id: number, content: string, _selection?: { start: number, end: number }) => {
    if (!currentProjectId) return;
    const now = Date.now();
        ...
        try {
            ...
            for (const [relativePath, zipEntryData] of Object.entries(unzipped) as [string, Uint8Array][]) {
                ...
                if (isBinaryFile) {
                    ...
                    const blob = new Blob([zipEntryData as BlobPart], { type: mimeType });
  ...
  const handleScanProjectFlow = useCallback(async () => {
    ...
    try {
        ...
        const report = await projectAnalysisService.analyzeProject(filesForCurrentProject, 
            ({ processed, total }: { processed: number; total: number }) => setScanStatus({ isScanning: true, scanType: 'shallow', processed, total })
        );
  ...
  return (
    <div className="w-full h-full bg-gray-900 flex flex-row overflow-hidden" onClick={closeAllContextMenus}>
                                        ...
                                        >
                                            ...
                                            {fileForPane ? (
                                                IMAGE_EXTENSIONS.includes(fileForPane.language) ? (
                                                    ...
                                                    <MonacoEditor
                                                        ref={(el: EditorRef | null) => { if(el) editorRefs.current[pane.id] = el; }}
                                                        file={fileForPane}
